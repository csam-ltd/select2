define([
 ...
 /**
            ...
            SelectAdapter.prototype.select = function (data) {
              ...
              if (self.options.options.filterLink) 
                  self.options.options.filterLink(data, self.$element[0].id);
              ...
              //If the id is the same as the text then this would indicate that we have a new tag
              if (data.id === data.text) {
                  data = self.selectNewTag(data);
              }
          ...
          /**
           * Add a tag which is new and has no id
           * @param {any} data
           */
          SelectAdapter.prototype.selectNewTag = function(data) {

              var self = this;

              //Create the new option for the html select
              var opt = document.createElement("option");
              opt.value = data.text;
              opt.selected = true;
              opt.text = data.text;

              //Add the new option to the component
              self.$element.get(0).add(opt);
